...
// XXX GeohazardVis: disable MSVC security warnings
#pragma warning(push)
...
vtkContext2DScalarBarActor::vtkContext2DScalarBarActor()
{
  ...
  // XXX GeohazardVis: No spacing between swatches (was 4.0). Visually it
  // doesn't make sense to have 100 or more colors with white spaces in between.
  this->Spacer = 0.0;
  // XXX End GeohazardVis
...
void vtkContext2DScalarBarActor::PaintColorBar(vtkContext2D* painter, double size[2])
{
  ...
  // XXX GeohazardVis: fetch nan color from concrete subclass
  double nanColor[4];
  nanColor[3] = 1.0;  // alpha value is not used (only set by vtkLookupTable)
  if (ctf)
  {
    ctf->GetNanColor(nanColor);
  }

  // XXX GeohazardVis: support vtkLookupTable
  if (!ctf)
  {
    // Mostly copy-paste of the code below, but without annotation support.
    auto lut = vtkLookupTable::SafeDownCast(this->LookupTable);
    if (!lut)
    {
      // this->LookupTable is not a ctf and not a lookup table, so nothing to
      // render here.
      return;
    }
    ...
    int numIndexedColors = lut->GetNumberOfTableValues();
    ...
    for (int i = 0; i < numIndexedColors; ++i)
    {
      ...
      lut->GetTableValue(i, indexedColor);
  ...
  // Continuous color map
  {
    ...
    // XXX GeohazardVis: skip if this->LookupTable is not a ctf
    if (!ctf)
    {
    }
    else
  ...
  if (this->DrawNanAnnotation)
  {
    ...
    // XXX GeohazardVis: don't access ctf here, it might not be valid
    brush->SetColorF(nanColor);
...
void vtkContext2DScalarBarActor::PaintAxis(vtkContext2D* painter, double size[2])
{
  ...
  // XXX GeohazardVis: stick to a low number of labels
  this->Axis->SetNumberOfTicks(this->GetNumberOfLabels());
  // XXX GeohazardVis
