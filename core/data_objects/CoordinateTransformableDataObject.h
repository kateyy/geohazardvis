 ...
 *
 * This class adds interfaces to request differently transformed coordinate data (geographic
 * global, metric global or local).
*/
...
public:
    ...
    */
     ...
     * coordinate systems.
     * If this returns missingInformation, additional information has to requested from the user
     * and supplied to specifyCoordinateSystem().
...
protected:
     ...
     * This may return an empty pointer, in which case the transformation is considered as not
     * supported.
    */
...
private:
    ...
    /**
     * The pipeline output m_pipelines[type, geoName, metricName, unit] converts the current data
     * set to a representation of in type, using the geographic coordinates geoName, and depending
     * on the type, coordinates in the metricName system.
     */
    QMap<CoordinateSystemType, QMap<QString, QMap<QString, QMap<QString, vtkSmartPointer<vtkAlgorithm>>>>> m_pipelines;
