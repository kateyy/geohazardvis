      ...
      vtkInformationVector *outputVector)
{
  ...
  // pass attribute arrays to the output
  ...
  magUpdateExt[2*comp[2]] = updateExt[2*comp[2]];
  ...
  magUpdateExt[2*comp[2]+1] = updateExt[2*comp[2]+1];
  ...
  int id = output->GetPointData()->AddArray(licOut);
  output->GetPointData()->SetActiveAttribute(id, vtkDataSetAttributes::SCALARS);
  licOut->Delete();
