#include "VectorMagnitudeMapping.h"
...
const bool VectorMagnitudeMapping::s_isRegistered = ScalarsForColorMappingRegistry::instance().registerImplementation(
    s_name,
    newInstances);

QList<ScalarsForColorMapping *> VectorMagnitudeMapping::newInstances(const QList<AbstractVisualizedData*> & visualizedData)
{
    ...
    QList<VectorMagnitudeMapping *> unchecked;

    for (const QString & arrayName : cellArrays)
        unchecked << new VectorMagnitudeMapping(withCellVectors, arrayName, vtkAssignAttribute::CELL_DATA);
    for (const QString & arrayName : pointArrays)
        unchecked << new VectorMagnitudeMapping(withPointVectors, arrayName, vtkAssignAttribute::POINT_DATA);

    QList<ScalarsForColorMapping *> instances;
    for (VectorMagnitudeMapping * mapping : unchecked)
    {
        if (mapping->isValid())
        {
            mapping->initialize();
...
VectorMagnitudeMapping::VectorMagnitudeMapping(
...
VectorMagnitudeMapping::~VectorMagnitudeMapping() = default;

QString VectorMagnitudeMapping::name() const
{
    return m_dataArrayName + " Magnitude";
}

vtkAlgorithm * VectorMagnitudeMapping::createFilter(AbstractVisualizedData * visualizedData)
{
...
bool VectorMagnitudeMapping::usesFilter() const
{
    return true;
}

void VectorMagnitudeMapping::configureMapper(AbstractVisualizedData * visualizedData, vtkMapper * mapper)
{
...
void VectorMagnitudeMapping::updateBounds()
{
